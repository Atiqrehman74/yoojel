...
export default function Sidebar({ open, onToggle, conversations, activeId, onSelect, onNew, onDelete }: Props) {
  ...
  if (!open) {
    return (
      <button
        onClick={onToggle}
        className="absolute left-3 top-3 z-30 rounded-lg p-2 text-gray-300 hover:bg-hover"
  ...
  return (
    <>
      ...
      <aside className="
        ...
        flex h-full w-[260px] flex-shrink-0 flex-col bg-sidebar text-gray-200
      ">
        {/* top: logo + collapse */}
        ...
        {/* footer: user account */}
        <div className="border-t border-white/10 p-3 pb-safe relative">
          <button
